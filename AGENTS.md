...
| `npm run test:all` | All tests (345 backend + 102 frontend = 447 total) |
...
- **State**: Composables (`src/composables/` — `useAgent`, `useToast`, `useSound`, `useVoiceInput`, `useWebSocket`) + `localStorage` (`agent-config` key). The `src/stores/` directory is currently empty.
