                ...
                <div class="token-chart-row">
                    <div class="token-donut">
                        <svg viewBox="0 0 100 100" class="donut-svg">
                            ...
                            <circle cx="50" cy="50" r="40" fill="none"
                                    :stroke="contextPercent > 90 ? 'var(--error)' : contextPercent > 70 ? 'var(--warning)' : 'var(--accent-primary)'"
                                    stroke-width="8"
                                    :stroke-dasharray="circumference"
                                    :stroke-dashoffset="circumference - (circumference * contextPercent / 100)"
                                    transform="rotate(-90 50 50)"
                                    class="donut-ring"/>
                            <text x="50" y="44" text-anchor="middle" class="donut-value">{{ formatNumber(tokenUsage.total) }}</text>
                    ...
                    <div class="token-bars">
                        <div class="mini-bar">
                            ...
                            <div class="bar-track">
                                <div class="bar-fill bar-prompt" :style="{ width: barPercent(tokenUsage.prompt) + '%' }"></div>
                            </div>
                        ...
                        <div class="mini-bar">
                            ...
                            <div class="bar-track">
                                <div class="bar-fill bar-completion" :style="{ width: barPercent(tokenUsage.completion) + '%' }"></div>
                            </div>
                        ...
                        <div class="mini-bar">
                            ...
                            <div class="bar-track">
                                <div v-if="tokenUsage.cached !== undefined && tokenUsage.cached > 0"
                                     class="bar-fill bar-cached"
                                     :style="{ width: barPercent(tokenUsage.cached) + '%' }"></div>
                                <span v-else class="bar-na">—</span>
                            </div>
                            <span class="bar-value">{{ tokenUsage.cached !== undefined ? formatNumber(tokenUsage.cached) : 'N/A' }}</span>
                        </div>
                    </div>
                </div>
